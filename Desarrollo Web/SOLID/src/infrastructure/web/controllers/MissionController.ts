...
import { InMemoryMissionRepository } from '../../persistence/InMemoryMissionRepository';
import { Mission } from '../../../core/domain/entities/Mission';
import { MissionStatus } from '../../../core/domain/value-objects/MissionStatus';

export class MissionController {
  private missionRepository: InMemoryMissionRepository;
  ...
  async updateMission(req: Request, res: Response): Promise<void> {
    try {
      ...
      } else if (!MissionStatus.isValidStatus(estado)) {
      ...
      // Actualizar misión
      existingMission.update(
        titulo.trim(),
        area_solicitante.trim(),
        parseInt(prioridad),
        parseFloat(costo_estimado),
        MissionStatus.fromString(estado)
      );
  ...
  async updateMissionStatus(req: Request, res: Response): Promise<void> {
    try {
      const id = parseInt(req.params.id as string);
      ...
      if (!MissionStatus.isValidStatus(estado)) {
        res.status(400).json({ 
          success: false,
      ...
      const updatedMission = await this.missionRepository.updateStatus(
        id,
        MissionStatus.fromString(estado)
      );
